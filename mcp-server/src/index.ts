...
const DEAL_BONUS: Record<string, number> = {
  Good: 10,
  Fair: 0,
  Over: -15,
};
...
type ListAllInputType = z.infer<typeof ListAllInput>;
...
Examples:
  ...
  async (params: ListAllInputType) => {
    try {
      const where: Record<string, string> = {};
      if (params.deal_filter !== "any") where.deal = params.deal_filter;
      if (params.condition_filter !== "any") where.condition = params.condition_filter;
      if (params.fuel_filter !== "any") where.fuelType = params.fuel_filter;
...
type ShortlistInputType = z.infer<typeof ShortlistInput>;
...
Examples:
  ...
  async (params: ShortlistInputType) => {
    try {
...
type GetDetailInputType = z.infer<typeof GetDetailInput>;
...
Error Handling:
  ...
  async (params: GetDetailInputType) => {
    try {
